...
> Markdown-Converter - Open source tool by Qyroxen

`cli` `conversion` `golang` `markdown` `utilities`

## What is it?

**Markdown Converter** is a cli tool designed for developers who need fast, reliable, and offline-capable tools. Built with Go for maximum performance and minimal resource usage.
...
### Install from source
```bash
git clone https://github.com/Qyroxen/Markdown-Converter.git
...
```bash
# Basic usage
./Markdown-Converter --path ./target

# With options
./Markdown-Converter --path ./target --format json --output report.json

# Verbose mode
./Markdown-Converter --path ./target --verbose
```
...
### Basic scan
```bash
./Markdown-Converter --path ./my-project
```

### JSON report
```bash
./Markdown-Converter --path ./my-project --format json --output report.json
```
...
- name: Run Markdown Converter
  run: ./Markdown-Converter --path . --format json --output report.json
```
